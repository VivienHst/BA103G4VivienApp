...
public class ProductTotalFragmentTest extends Fragment {
    ...
    List<ProdVO> getProductList(){
        ...
        if (networkConnected()) {
            retrieveProdTask = new RetrieveTotalProdTask().execute(Common.PROD_URL);
            try {
    ...
    class RetrieveTotalProdTask extends AsyncTask<String, Void, List<ProdVO>>{
